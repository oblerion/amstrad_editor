  ...
  Title(lang);
   ...
   else if(strfind(mem,"close")==0)
   {
    ...
    system(C_clear.c_str());
     ...
     {
      tt= C_del + " " + strsub(mem,4,mem.size()) + "/.";
      system(tt.c_str());
      tt=  C_del_dir +" "+ strsub(mem,4,mem.size());
      system(tt.c_str());
     }
     else if(strfind(mem,"copy") == 0)
     {
       int pos1 = atoi(strsub(mem,4,strfind(mem,"to")).c_str());
       int pos2 = atoi(strsub(mem,strfind(mem,"to")+2,mem.size()).c_str());
       string memo;
       memo = strsub(mem,strfind(mem,"to")+2,mem.size());
       printf("%s",memo.c_str());
     ...
     else if(strfind(mem,"save") == 0)
     {
      fic.open("export.amd",ios::out);
       ...
       fic.close();
     if(mem.size() <= 5) {tt=C_move+" export.amd " + file;}
       else tt= C_move + " export.amd " + strsub(mem,5,mem.size());
       system(tt.c_str());
     }
     else if(strfind(mem,"load")==0)
     {
      ...
      if(strfind(mem,"/")!=-1) 
      {
        ...
        tt = C_copy + " " + strsub(mem,5,mem.size()-file.size()+2) + " .";// + file;
        puts(tt.c_str());
        system(tt.c_str());
        tt = C_move + " " + file + " main.amd";
        system(tt.c_str());
        tt = C_copy+" main.amd main2.amd";
        system(tt.c_str());
        tt = C_move+" main2.amd " + strsub(mem,4,mem.size()) ;
        system(tt.c_str());   
      ...
      file = strsub(mem,5,mem.size());
        tt = C_copy + " " + strsub(mem,5,mem.size()) + " main.amd";
        system(tt.c_str());
      }
      string eee;
      ...
      if(fic.is_open())
      {
        for(int i=0;i<lignelimit-1;i++)
        {
          ...
          {
          char* tst;
          tst = (char*)malloc(eee.size());
          sprintf(tst,"%d %s",i,eee.c_str());
            tab[i] = tst;
            free(tst);
          }
        }
        fic.close();
      system(( C_del + " main.amd").c_str());
      printf("fichier : %s\n",file.c_str());
      List(tab);
      }
      else
    ...
    system(C_clear.c_str());
     ...
     else if(atoi(mem.c_str()) >= 0 && atoi(mem.c_str()) < lignelimit && strfind(mem," ")==-1)
     {
       ...
       if(num < lignelimit) {tab[num].clear();
        system(C_clear.c_str());
        printf("fichier : %s\n",file.c_str());List(tab); }
     }
     else if(mem[1] == ' ' && atoi(strsub(mem,0,1).c_str()) > -1) 
     {
       num = atoi(strsub(mem,0,1).c_str());
       if(strfind(mem,"\n") != -1) mem = strsub(mem,0,strfind(mem,"\n"));
       if(num < lignelimit) {tab[num]= mem; system(C_clear.c_str());printf("fichier : %s\n",file.c_str());List(tab);}
     }
